...
class TCP_Client(cmd.Cmd):
    ...
    def do_GET(self, person):
        ...
        if person:
           os.system('python client_tcp.py %s %d GET %s' %(self.hostname, int(self.port),person))
        else:
    ...
    def do_QUERY(self, line):
        ...
        os.system('python client_tcp.py %s %d QUERY all' %(self.hostname, int(self.port)) )
    ...
    def do_PUT(self, person_number):
        ...
        Add key/value to DB"""
        if person_number:
           os.system('python client_tcp.py %s %d PUT %s'%(self.hostname, int(self.port),person_number))
        else:
    ...
    def do_DELETE(self, person):
        ...
        if person:
           os.system('python client_tcp.py %s %d DELETE %s' %(self.hostname, int(self.port), person))
        else:
...
if __name__ == '__main__':
    logging.basicConfig(filename='client_tcp.log', level=logging.INFO)
    ...
    TCP_Client().do_PUT("person1 number1")
    TCP_Client().do_PUT("person2 number2")
    TCP_Client().do_PUT("person3 number3")
    TCP_Client().do_PUT("person4 number4")
    TCP_Client().do_PUT("person5 number5")
    ...
    TCP_Client().do_GET('person1')
    TCP_Client().do_GET('person2')
    TCP_Client().do_GET('person3')
    TCP_Client().do_GET('person4')
    TCP_Client().do_GET('person5')
    ...
    TCP_Client().do_DELETE('person1')
    TCP_Client().do_DELETE('person2')
    TCP_Client().do_DELETE('person3')
    TCP_Client().do_DELETE('person4')
    TCP_Client().do_DELETE('person5')

#   doing initial setup of ports, hostname, and instance
    TCP_Client().port = 9999
    TCP_Client().hostname = 'localhost'
    TCP_Client().instance = 'SOCK_STREAM'
